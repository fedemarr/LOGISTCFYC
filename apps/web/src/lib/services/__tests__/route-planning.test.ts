...
describe("deleteRoute / completeRouteManually (pedido de Fede, integración contra Supabase real)", () => {
  ...
  async function makeApprovedRoute(label: string, packageCount: number) {
    ...
    const [vehicle] = await db
      ...
      .values({
        orgId,
        plate: `DEL-${label.slice(0, 4).toUpperCase()}-${randomUUID().slice(0, 4)}`,
        status: "AVAILABLE",
        assignedDriverId: driverId,
        capacityPackages: 10,
      })
      .returning();
    if (!vehicle) throw new Error("no se pudo crear el vehículo de test");
    ...
    for (let i = 0; i < packageCount; i++) {
      const [addr] = await db
        ...
        .values({
          orgId,
          normalizedHash: `route-del-${runId}-${label}-${i}`,
          rawText: `Calle Borrar ${label} ${i}`,
          lat: villaBallester.lat + i * 0.001,
          lng: villaBallester.lng + i * 0.001,
          geocodeAccuracy: "ROOFTOP",
        })
        .returning();
      if (!addr) throw new Error("no se pudo crear la dirección de test");
      await db.insert(packages).values({
        orgId,
        operationId: op.id,
        internalCode: `ML-DEL-${runId}-${label}-${i}`,
        trackingCode: `TC-DEL-${runId}-${label}-${i}`,
        status: "GEOCODIFICADO",
        addressId: addr.id,
      });
    }
